...
const location = useLocation();
  ...
  const handleRightArrowPress = () => {
    if (arrposition < arrlength - 1) { // Check if the position is less than the last index
      setArrposition((prev) => prev + 1);
    } else {
  ...
  const fetchData = async (position) => {
    try {
      ...
      const data = await
        fetch(`https://41c664jpz1.execute-api.us-west-1.amazonaws.com/dev/userinfo/${uid}`)
...
const styles = {
  ...
  redCircle: {
    ...
    backgroundColor: 'white', // Background stays white
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
